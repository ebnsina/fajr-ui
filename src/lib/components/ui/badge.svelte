...
<script lang="ts">
	...
	let { class: className, variant, size, href, children, ...rest }: BadgeProps = $props();
</script>

{#if href !== undefined}
	<a {href} data-slot="badge" class={cn(badgeVariants({ className, size, variant }))} {...rest}>
		{@render children?.()}
	</a>
{:else}
	<span data-slot="badge" class={cn(badgeVariants({ className, size, variant }))} {...rest}>
		{@render children?.()}
	</span>
{/if}
